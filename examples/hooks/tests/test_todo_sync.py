...
from todo_sync_workflow import main, sync_todos_to_doob  # noqa: E402


class TestSyncTodosToDoob:
    """Tests for sync_todos_to_doob function."""

    @patch("todo_sync_workflow.subprocess.run")
    def test_syncs_pending_todo(self, mock_run: MagicMock) -> None:
        """Should sync a pending todo to doob."""
        mock_run.return_value = MagicMock(returncode=0)
        todos = [{"content": "Test task", "status": "pending"}]

        synced, errors = sync_todos_to_doob(todos)

        assert synced == 1
        assert errors == 0
        mock_run.assert_called_once()
        call_args = mock_run.call_args[0][0]
        assert call_args[:3] == ["doob", "todo", "add"]
        assert "Test task" in call_args
        assert "-t" in call_args

    @patch("todo_sync_workflow.subprocess.run")
    def test_syncs_in_progress_todo(self, mock_run: MagicMock) -> None:
        """Should sync an in_progress todo to doob."""
        mock_run.return_value = MagicMock(returncode=0)
    ...
    def test_handles_timeout(self, mock_run: MagicMock) -> None:
        ...
        mock_run.side_effect = subprocess.TimeoutExpired(cmd="doob", timeout=5)
    ...
    @patch("todo_sync_workflow.subprocess.run")
    def test_handles_missing_doob(self, mock_run: MagicMock) -> None:
        """Should handle missing doob command gracefully."""
...
class TestMain:
    ...
    @patch("todo_sync_workflow.sync_todos_to_doob")
