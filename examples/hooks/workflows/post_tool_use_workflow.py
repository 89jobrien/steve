...
# dependencies = ["pyyaml"]
# ///
"""PostToolUse workflow - orchestrates code quality analyzers after tool execution.

ARCHITECTURE: Thin orchestrator that composes analyzers from analyzers/ directory.

Orchestrates:
  - analyzers/lint_changed.py - Lints modified files
  - analyzers/typecheck_changed.py - Type checks modified files
  - analyzers/security_audit.py - Scans for security issues
  - analyzers/todo_tracker.py - Tracks TODO/FIXME comments
  - analyzers/complexity_checker.py - Checks code complexity
  - analyzers/import_validator.py - Validates import statements

Exit codes:
  0 = All checks passed
  1 = Warnings detected (operation already completed)

This workflow should remain < 150 lines. Complex logic belongs in analyzers/.
"""
