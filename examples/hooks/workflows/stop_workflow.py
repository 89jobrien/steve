...
# dependencies = ["pyyaml"]
# ///
"""Stop workflow - orchestrates end-of-session lifecycle tasks.

ARCHITECTURE: Thin orchestrator that composes lifecycle hooks from lifecycle/ directory.

Orchestrates:
  - lifecycle/metrics_collector.py - Collects session metrics
  - lifecycle/commit_suggester.py - Suggests git commits
  - lifecycle/knowledge_update.py - Updates knowledge bases
  - lifecycle/cleanup_handler.py - Cleanup old sessions
  - lifecycle/export_conversation.py - Exports transcript

Exit codes:
  0 = Success (Stop hooks cannot block termination)

This workflow should remain < 150 lines. Complex logic belongs in lifecycle/.
"""
