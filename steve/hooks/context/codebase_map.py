...
def matches_include_pattern(entry: Path, include_patterns: list[str], project_root: Path) -> bool:
    ...
    for pattern in include_patterns:
        ...
        if pattern_normalized.endswith("/**"):
            ...
            if rel_str_forward == dir_pattern or rel_str_forward.startswith(dir_pattern + "/"):
                return True
...
def generate_tree(
    ...
    entries = [e for e in entries if matches_include_pattern(e, include_patterns, project_root)]
...
def main() -> None:
    with hook_invocation("codebase_map") as inv:
        ...
        codebase_map = generate_tree(project_root, max_depth, include_patterns, project_root)
