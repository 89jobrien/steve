    ...
    def test_logs_to_daily_jsonl_and_breadcrumb(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                with (
                    patch.dict(os.environ, {"CLAUDE_HOOK_LOG_DIR": str(log_dir)}, clear=False),
                    patch.object(hook_logging, "datetime") as dt,
                ):
                    dt.now.side_effect = [fixed_start, fixed_end]
                    with (
                        patch.object(hook_logging.time, "monotonic", side_effect=[10.0, 10.123]),
                        patch.object(sys, "stderr", stderr),
                        hook_logging.hook_invocation("secret_scanner") as inv,
                    ):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                with (
                    ...
                    dt.now.side_effect = [fixed_start, fixed_end]
                    with patch.object(hook_logging.time, "monotonic", side_effect=[10.0, 10.010]):
                        stderr = io.StringIO()
                        with patch.object(sys, "stderr", stderr):
                            with self.assertRaises(SystemExit) as cm:
                                with hook_logging.hook_invocation("dangerous_command_guard"):
                                    raise SystemExit(2)
