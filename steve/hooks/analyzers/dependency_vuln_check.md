...
description: Check for dependency vulnerabilities when package files change.
...
async def check_pip_audit(cwd: str) -> tuple[bool, str]:
    ...
    if exit_code != 0:
        ...
        vuln_count = len(
            [line for line in lines if line.strip() and not line.startswith("Name")]
        )
        if vuln_count > 0:
...
def main() -> None:
    with hook_invocation("dependency_vuln_check") as inv:
        ...
        if not ok:
            ...
            print(
                "    - Review vulnerabilities and update dependencies", file=sys.stderr
            )
