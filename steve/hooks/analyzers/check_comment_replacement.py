...
def is_comment_replacement(old_string: str, new_string: str) -> bool:
    ...
    old_code_lines = [line for line in old_lines if line.strip() and not is_comment(line)]
    new_code_lines = [line for line in new_lines if line.strip() and not is_comment(line)]
    ...
    if len(old_code_lines) >= 3 and len(new_code_lines) == 0 and len(new_comment_lines) > 0:
        return True
