...
def detect_test_runner(project_root: Path) -> str | None:
    if (
        (project_root / "pytest.ini").exists() or (project_root / "pyproject.toml").exists()
    ) and command_exists("pytest"):
        return "pytest"

    if (
        (project_root / "jest.config.js").exists() or (project_root / "jest.config.ts").exists()
...
async def run_tests(test_runner: str, test_files: list[str], cwd: str) -> tuple[int, str, str]:
