...
def detect_linters(project_root: Path) -> list[str]:
    ...
    if (
        (project_root / "biome.json").exists() or (project_root / "biome.jsonc").exists()
    ) and command_exists("biome"):
        linters.append("biome")

    if (
        (project_root / ".eslintrc.js").exists() or (project_root / ".eslintrc.json").exists()
    ) and command_exists("eslint"):
        linters.append("eslint")

    if (
        (project_root / "ruff.toml").exists() or (project_root / "pyproject.toml").exists()
...
async def main_async(payload: dict) -> int:
    ...
    failed_linters = [(linter, code, out, err) for linter, code, out, err in results if code != 0]

    if failed_linters:
        print("[Error] Linting failed", file=sys.stderr)
        print(f"  Details: {len(failed_linters)} linter(s) found issues", file=sys.stderr)
