...
def run_linter(file_path: str, cwd: str) -> AnalysisResult:
    ...
    # JS/TS files
        ...
        for linter in [["npx", "biome", "check", file_path], ["npx", "eslint", file_path]]:
            try:
                result = subprocess.run(
                    linter, check=False, cwd=cwd, capture_output=True, text=True, timeout=30
                )
                if result.returncode != 0 and result.stdout:
    ...
    return AnalysisResult(name="linter", passed=len(messages) == 0, messages=messages)
...
def check_complexity(content: str, file_path: str) -> AnalysisResult:
    ...
    return AnalysisResult(name="complexity", passed=max_depth <= 6, messages=messages)
