# Research Report: {{TOPIC}}

**Generated:** {{TIMESTAMP}}
**Query Type:** {{BREADTH|DEPTH|FACTUAL}}
**Confidence:** {{HIGH|MEDIUM|LOW}}

---

## Executive Summary

{{3-5_PARAGRAPH_OVERVIEW}}

---

## Key Findings

1. **{{FINDING_1}}**
   - Evidence: {{SUPPORTING_DATA}}
   - Sources: {{SOURCE_COUNT}} sources

2. **{{FINDING_2}}**
   - Evidence: {{SUPPORTING_DATA}}
   - Sources: {{SOURCE_COUNT}} sources

3. **{{FINDING_3}}**
   - Evidence: {{SUPPORTING_DATA}}
   - Sources: {{SOURCE_COUNT}} sources

---

## Detailed Analysis

### {{THEME_1}}

{{COMPREHENSIVE_ANALYSIS}}

#### Supporting Evidence

- {{EVIDENCE_1}} ([Source]({{URL}}))
- {{EVIDENCE_2}} ([Source]({{URL}}))

#### Implications

{{WHAT_THIS_MEANS}}

---

### {{THEME_2}}

{{COMPREHENSIVE_ANALYSIS}}

#### Supporting Evidence

- {{EVIDENCE_1}} ([Source]({{URL}}))
- {{EVIDENCE_2}} ([Source]({{URL}}))

---

### {{THEME_3}}

{{COMPREHENSIVE_ANALYSIS}}

---

## Comparisons

| Aspect | {{OPTION_A}} | {{OPTION_B}} | {{OPTION_C}} |
|--------|--------------|--------------|--------------|
| {{CRITERIA_1}} | {{VALUE}} | {{VALUE}} | {{VALUE}} |
| {{CRITERIA_2}} | {{VALUE}} | {{VALUE}} | {{VALUE}} |
| {{CRITERIA_3}} | {{VALUE}} | {{VALUE}} | {{VALUE}} |

---

## Timeline

| Date | Event |
|------|-------|
| {{DATE}} | {{EVENT}} |
| {{DATE}} | {{EVENT}} |

---

## Gaps & Uncertainties

### Information Gaps

- {{GAP_1}}: Unable to verify due to {{REASON}}
- {{GAP_2}}: Conflicting information from sources

### Contradictions Found

| Claim | Source A Says | Source B Says |
|-------|---------------|---------------|
| {{CLAIM}} | {{POSITION}} | {{POSITION}} |

### Areas Requiring Further Research

1. {{AREA_1}}
2. {{AREA_2}}

---

## Recommendations

### Immediate Actions

1. {{ACTION_1}}
2. {{ACTION_2}}

### Further Investigation Needed

1. {{INVESTIGATION_1}}
2. {{INVESTIGATION_2}}

---

## Sources & References

### Primary Sources

1. [{{TITLE}}]({{URL}}) - {{DESCRIPTION}}
2. [{{TITLE}}]({{URL}}) - {{DESCRIPTION}}

### Secondary Sources

1. [{{TITLE}}]({{URL}}) - {{DESCRIPTION}}

### Academic / Technical

1. {{AUTHOR}} ({{YEAR}}). {{TITLE}}. {{PUBLICATION}}. [URL]({{URL}})

---

## Research Methodology

| Metric | Value |
|--------|-------|
| Query Classification | {{TYPE}} |
| Research Agents Used | {{N}} |
| Total Searches | {{N}} |
| Sources Analyzed | {{N}} |
| Research Duration | {{TIME}} |

### Research Artifacts

- `/tmp/research_{{TOPIC_1}}.md`
- `/tmp/research_{{TOPIC_2}}.md`
- Final: `/tmp/research_final_{{TIMESTAMP}}.md`

---

## Quality Checklist

- [ ] Multiple independent sources confirm key findings
- [ ] Contradictions and gaps documented
- [ ] Primary sources prioritized over secondary
- [ ] Methodology transparent and reproducible
- [ ] Confidence levels appropriate to evidence
- [ ] Actionable recommendations provided
